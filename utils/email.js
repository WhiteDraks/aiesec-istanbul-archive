...
/**
 * Kullanıcı ilk kayıt olduğunda bekleme süreci maili gönderir
 * @param {string} email - Kullanıcı e-posta adresi
 * @param {string} name - Kullanıcı adı
 */
async function sendWelcomePendingEmail(email, name) {
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      subject: 'AIESEC İstanbul Mezunlar Portalı - Kayıt Talebiniz Alındı 📩',
      html: `
    ...
    console.log(`Kayıt bilgilendirme maili gönderildi: ${email}`);
    return true;
  } catch (err) {
    console.error('Mail gönderim hatası:', err);
    return false;
